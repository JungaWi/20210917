...
//		Student s3 = new People(); -> 부모클래스는 자식클래스에 담을 수 없다
//		p2.getStudentNo(); -> 부모클래스 변수에는 자식클래스 메소드 호출이 불가능
		if (p2 instanceof Student) { // p2에 들어있는 값이 student 인스턴스
			Student s3 =(Student) p2;
			s3.getStudentNo();
		}
		if (p1 instanceof Student) {
			Student s4 = (Student) p1; // p1에 들어있는 값이 student 인스턴스가 아니라서 에러 -> if 사용
			s4.getStudentNo();		
		} else {
